...
test("statically renders the finished Twilight Leap entry screen", async () => {
...
test("ships production art and removes starter-only files", async () => {
  ...
  await Promise.all([
    ...
    access(new URL("../out/assets/twilight-leap-atlas.png", import.meta.url)),
    access(new URL("../out/icon.svg", import.meta.url)),
    access(new URL("../out/og.png", import.meta.url)),
  ]);
  await Promise.all(
    ["favicon.svg", "file.svg", "globe.svg", "window.svg"].map((name) =>
      assert.rejects(access(new URL(`../out/${name}`, import.meta.url))),
    ),
  );
